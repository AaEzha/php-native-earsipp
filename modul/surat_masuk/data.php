		...
		<table class="table table-borderd table-hovered table-striped" cellspacing="0" width="100%">
			...
			?>
				<tr>
					...
					<td><?= $data['perihal'] ?></td>
					<td><?= $data['kepada_surat_masuk'] ?></td>
					<td><?= $data['pic_surat_masuk'] ?></td>
					...
					<td>
						...
						<a href="?halaman=surat_masuk&hal=hapus&id=<?= $data['id_surat_masuk'] ?>" class="btn btn-danger" onclick="return confirm('Apakah yakin ingin menghapus data ini?')">Hapus </a>
					</td>
				</tr>
			<?php endwhile; ?>
		</table>
